...
function part1() {
  const numericInput = parseInput("input2.txt");
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
      path[i][j] = path[i][j].sort((a, b) => {
        ...
        if (
          //!path[i][j].includes("Right") &&
          //!path[i][j].includes("Down") &&
          //path[i][j].filter((x) => x === "Up").length > 1 &&
          j == 0
        ) {
          console.log("includes both", path[i][j]);
          return 0;
        }
